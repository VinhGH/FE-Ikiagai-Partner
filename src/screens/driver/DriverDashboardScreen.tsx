...
export default function DriverDashboardScreen() {
  ...
  const handleAccept = (order: IncomingOrder) => {
    // Navigate to CurrentDelivery screen in real app
    setCurrentOrder(null);
  ...
  const handleDecline = (_order: IncomingOrder) => {
    setCurrentOrder(null);
    // Auto-show new order after 5s in real app
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {/* ── Online/Offline Toggle — outside ScrollView to avoid stickyHeaderIndices native driver conflict ── */}
...
const styles = StyleSheet.create({
  ...
  waitingIcon: { fontSize: 40, marginBottom: SPACING.md },
  waitingTitle: {
    fontSize: SIZES.h4,
    fontWeight: '700',
    color: COLORS.text,
